...
CMI.estimates = function(data, xind, yind, zinds, eps=0, Kmax=0, v=F, isCat=c(), noVol=T, logE=T, number_cores_use = 7){
    max_ups = 5
    ...
    if(length(zinds) > 0){
      ...
      resXYZ = iterative_cmi_greedy_flexible_parallel(data=as.matrix(data[,inds]), isCat=c(1:length(inds))[c(inds %in% isCat)], eps = eps, Kmax = Kmax, max_num_updating=max_ups, cores=number_cores_use)
      xsplit = 1
      ysplit = 2
